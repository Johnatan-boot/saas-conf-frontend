...
import {
  ...
  Bell, Cake, Menu, X, FileText, Crown, Trash2, Brain,
  Calculator, ShoppingCart, RefreshCw, DollarSign
} from 'lucide-react'
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const visibleNav = NAV.filter(n => {
    if (n.superAdminOnly) return user?.role === 'super_admin' || user?.email?.includes('admin')
